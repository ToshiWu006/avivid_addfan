(async function () {
    ...
    AviviD.update_couponUI = async function (is_exit = false, timeset = 30) {
        ...
        if (product_name !== '_') {
            var product_div = `<a class=avivid_coupon_title_product href="` + product_url + `" onclick="AviviD.AcceptCoupon(true, is_exit=` + is_exit + `)"><div>` + product_name + `限時優惠</div></a>`;
    ...
    AviviD.Promotion_coupons = function (title, content, code, timeset, limit, mode = 0, is_exit = false) {
        ...
        if(AviviD.get_cookie_tracking('AviviD_is_coupon') == '1' || AviviD.get_cookie_tracking('AviviD_is_coupon_b') == '1'){
