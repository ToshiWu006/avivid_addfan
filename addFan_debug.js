(async function(){
    AviviD.check_addfan_url = function(block_urls) {
        var pathname = location.pathname;
        if (pathname==='/') {
            return true;
        };
        var url_match = block_urls.find(x => x.match(new RegExp(pathname))); // str or undefined
        return url_match===undefined ? true : false;
    };  
    var block_urls = ['https://mob.i3fresh.tw/shoppingcart.html', 'https://mob.i3fresh.tw/setorder.html', 'https://mob.i3fresh.tw/moborderok.html', 'https://mob.i3fresh.tw/order.html']
    //// load only when now url not in block url list
    if (AviviD.check_addfan_url(block_urls)) {
        //// cart parser
